...
import { Plus, Search, MoreVertical, Edit, Trash2, Package, Loader2, Upload, ImageIcon } from "lucide-react";
...
export default function ProductsPage() {
    ...
    async function handleSave() {
        ...
        try {
            ...
            alert("Erro ao salvar produto. Verifique se você fez login e tem permissão.");
        } finally {
            setSaving(false);
        }
    }
        ...
        <div className="flex min-h-screen flex-col bg-gray-50 dark:bg-gray-900">
            <header className="sticky top-0 z-10 border-b bg-background px-6 py-4 flex items-center justify-between">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground">Gerencie o cardápio do seu evento</p>
                </div>
                <div className="flex gap-4">
                    <div className="relative w-64">
                        <Search className="absolute left-2 top-2.5 h-4 w-4 text-muted-foreground" />
                        <Input placeholder="Buscar produto..." className="pl-8" value={searchTerm} onChange={e => setSearchTerm(e.target.value)} />
                    </div>
                    <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
                        ...
                        <DialogContent className="sm:max-w-[500px]">
                            <DialogHeader>
                                ...
                                <DialogDescription>
                                    Preencha os detalhes do produto e adicione uma imagem.
                                </DialogDescription>
                            </DialogHeader>
                            <div className="grid gap-4 py-4">
                                ...
                                <div className="flex flex-col items-center justify-center gap-4 mb-4">
                                    <div
                                        className="relative flex h-32 w-32 cursor-pointer items-center justify-center rounded-xl border-2 border-dashed border-muted-foreground/25 bg-muted/50 hover:bg-muted transition-colors overflow-hidden"
                                    ...
                                    <p className="text-xs text-muted-foreground text-center">
                                        Clique para enviar uma imagem (PNG, JPG)
                                    </p>
                                </div>
                                ...
                                <div className="grid grid-cols-4 items-center gap-4">
                                    ...
                                    <Select onValueChange={setNewCategoryId} value={newCategoryId}>
                                ...
                                {loading ? (
                                    ...
                                    filteredProducts.map((product) => (
                                        <TableRow key={product.id}>
                                            ...
                                            <TableCell>
                                                <span className={`font-mono ${product.stock <= 5 ? 'text-red-500 font-bold' : ''}`}>
                                                    {product.stock}
                                                </span>
                                            </TableCell>
                                            <TableCell>
