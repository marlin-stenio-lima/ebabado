...
import { DollarSign, ArrowLeft, CheckCircle2, RefreshCcw } from "lucide-react";
...
export function ChangeCalculator({ fixedTotal, onFinish, onBack }: ChangeCalculatorProps) {
    ...
    useEffect(() => { if (fixedTotal) setTotal(fixedTotal); }, [fixedTotal]);
    useEffect(() => {
        if (received >= total && total > 0) setChange(received - total);
        else setChange(null);
    }, [total, received]);

    const reset = () => { setTotal(0); setReceived(0); setChange(null); };
        ...
        <Card className="w-full max-w-sm mx-auto shadow-none border-0">
            ...
            <CardContent className="space-y-4 px-0">
                <div className="space-y-2">
                    ...
                    <Input id="total" type="number" placeholder="0.00" value={total || ""} onChange={(e) => setTotal(Number(e.target.value))} className="text-lg font-mono bg-muted/50" step="0.01" readOnly={!!fixedTotal} />
                </div>
                <div className="space-y-2">
                    <Label htmlFor="received">Valor Recebido (R$)</Label>
                    <Input id="received" type="number" placeholder="0.00" value={received || ""} onChange={(e) => setReceived(Number(e.target.value))} className="text-lg font-mono border-primary/50 focus:border-primary" step="0.01" autoFocus />
                </div>
                <div className={`p-4 rounded-lg text-center transition-colors ${change !== null ? "bg-green-100 text-green-800 dark:bg-green-900/30 dark:text-green-400" : "bg-muted/50 text-muted-foreground"}`}>
                    ...
                    <div className="text-3xl font-bold font-mono">{change !== null ? `R$ ${change.toFixed(2)}` : "---"}</div>
                </div>
                {onFinish ? (
